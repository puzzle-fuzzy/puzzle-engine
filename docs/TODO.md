...
- Canvas phase 的纯业务逻辑从 server modules 拆到 package 或 domain service，worker 不再动态 import server 文件。`canvas.analyze`、`canvas.characters`、`canvas.locations`、`canvas.characterRefs`、`canvas.locationRefs`、`canvas.storyboard`、`canvas.continuity`、`canvas.rebuild`、`canvas.videos` 已完成 worker 去 server service；Canvas 视频提交 runtime 已完成 server/worker 复用。剩余工作从“去 server 动态 import”转为继续抽 Canvas domain service、资产编排 facade、task/run DB adapter。
