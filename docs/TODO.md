...
状态：部分完成，`packages/prompt-engine` 已完成 Canvas prompt 模板、LLM JSON parser、shot video prompt builder 的基础拆分（commit：`refactor(prompt-engine): extract canvas prompt utilities`）；`packages/canvas-engine` 已完成镜头连续性校验规则基础拆分（commit：`refactor(canvas-engine): extract continuity rules`）以及 analysis/character/location/storyboard 的 LLM 输出 schema 校验器（commit：`e62cee1`、`refactor(server,worker): validate canvas LLM output before persisting`），server 与 worker 的 10 个 `parseLLMJson` 调用点已全部走校验器收口。Canvas 阶段执行的共享核心正在下沉到 `packages/canvas-runtime`（消除 server fire-and-forget 与 worker task-driven 两条路径的逐行复制）：analysis/characters/locations/storyboard 四个文本阶段的核心已抽到 `canvas-runtime/src/phases/`，server 与 worker 改为调用同一核心（`runAnalysisPhase`/`generateCharacterEntity`/`generateLocationEntity`/`runStoryboardPhase`），并顺带把高频复制的 prompt→validate→chat 段收口为 `runTextLlmOnce`、把 `toNormalized*` 归一化映射收口为 `canvas-runtime/normalize.ts`；同时修复 server `locations.ts` 用 name 而非 location.id 做 completed 通知的 bug、把 worker storyboard 状态写对齐到 lambda 外。continuity/rebuild 两个纯计算阶段的核心也已下沉（`runContinuityPhase` 收口 validateShotContinuity + createContinuityReport、`buildShotVideoPromptEntity` 收口 per-shot buildShotVideoPrompt），server `continuity-rebuild.ts` 与 worker `canvas-continuity.ts`/`canvas-rebuild.ts` 改为调用同一核心，server 内联的 normalized 映射与 worker 的 `toNormalized*` 调用一并删除，rebuild 的无 per-shot try/catch 行为保持不变。剩余：character-refs/location-refs/videos 媒体核心后续继续拆。
