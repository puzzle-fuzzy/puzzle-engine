...
状态：部分完成，`packages/task-engine` 已完成 retry/error 分类和 handler registry 基础拆分（commit：`2c0d727`、`refactor(task-engine): add handler registry`），`packages/workflow-engine` 已完成 Canvas phase 顺序、task type 映射、自动推进决策的基础拆分（commit：`refactor(workflow-engine): extract canvas phase rules`）。剩余：worker 仍负责 DB 适配、run/task 创建，且 Canvas handler 仍动态加载 server service，后续需要继续抽 Canvas domain service。
